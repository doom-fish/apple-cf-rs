...
//!
//! This module provides the minimum-viable Metal bridge: a [`MetalDevice`]
//! handle, a [`MetalTexture`] wrapper, and an extension trait that lets
//! [`IOSurface`] hand out Metal textures with no CPU copy.
//!
//! For higher-level rendering (built-in shaders, YCbCr-to-RGB conversion,
//! multi-plane texture descriptors), see `screencapturekit::metal`.
...
pub trait IOSurfaceMetalExt {
    ...
    fn create_metal_texture(&self, device: &MetalDevice, plane_index: usize)
        -> Option<MetalTexture>;
}
